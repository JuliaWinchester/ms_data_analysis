...
for s in r:
	if s['uuid']:
		...
		json = resp.json()
		# if get_db_field(s, 'genus').lower() != get_json_field(json, ['indexTerms', 'genus']).lower():
		# 	print 'MS genus: ' + get_db_field(s, 'genus') + '     iDigBio genus: ' + get_json_field(json, ['indexTerms', 'genus'])
		if get_db_field(s, 'genus').lower() == get_json_field(json, ['indexTerms', 'genus']).lower():
				if get_db_field(s, 'genus').lower() != get_json_field(json, ['data', 'dwc:genus']).lower():
					print 'MS: ' + get_db_field(s, 'genus') + '     iDigBio: ' + get_json_field(json, ['data', 'dwc:genus'])
		# if get_db_field(s, 'species').lower() != get_json_field(json, ['indexTerms', 'specificepithet']).lower():
		# 	print 'MS species: ' + get_db_field(s, 'species') + '     iDigBio species: ' + get_json_field(json, ['indexTerms', 'specificepithet'])
